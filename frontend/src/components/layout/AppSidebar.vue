...
const links = computed(() => {
  ...
  if (props.role === 'admin' || props.role === 'administrativo') {
    return [
      ...
      { to: { name: 'admin-mail' },        label: 'Correo masivo',   icon: 'mail' },
      { to: { name: 'admin-analytics' },   label: 'Analytics',       icon: 'analytics' },
      { to: { name: 'admin-settings' },    label: 'Configuración',   icon: 'cog' },
    ]
  }
  return []
})
    ...
    <nav class="flex-1 p-2 space-y-1 flex flex-col">
      ...
      >
        ...
        <span class="shrink-0 w-5 h-5 flex items-center justify-center">
          ...
          <!-- mail -->
          <svg v-else-if="link.icon === 'mail'" class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
          </svg>
          <!-- cog -->
          <svg v-else-if="link.icon === 'cog'" class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
